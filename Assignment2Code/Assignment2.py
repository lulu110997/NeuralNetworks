...
def QuestionOne():
	...
	wp = np.array([[-0.2206, 0.2139, 0.4764, -0.1886, 0.5775, -0.7873, -0.2943, 0.9803, -0.5945, -0.2076, 0.7611, 0.9635, -0.1627, -0.0503, 0.3443, -0.4812, 0.8625, 0.3333, 0.1565, 0.7611, 0.3211, 0.7651, -0.9820, 0.2245, 0.3536, -0.1562],
		[0.1932, 0.8436, -0.6475, 0.3365, 0.1795, -0.0542, 0.6263, -0.7222, -0.6026, 0.3556, -0.2030, -0.0680, 0.6924, 0.5947, 0.6762, 0.2802, -0.1763, 0.5520, 0.2588, 0.3365, 0.7628, -0.5172, 0.3358, 0.7878, -0.1101, 0.8217],
...
class Model(nn.Module):
	def __init__(self, channel=1, imageSize=50):
		...
		out3 = 128
		out4 = 128
		self.conv1 = nn.Conv2d(self.channel, out1, 5, stride=1) 
...
def QuestionTwo():
	...
	sched = optim.lr_scheduler.StepLR(optimizer, 10, 0.5) 
	...
	for epoch in tqdm(range(epochs)):
		...
		if (epoch_acc[epoch+1,:].item() > epoch_acc[:epoch+1]).all():
			...
			torch.save(CNNModel.state_dict(), 'C:\\Users\\louis\\OneDrive\\Desktop\\NN\\assignment2\\cd50filesGreyscaledResults\\test.pt')
